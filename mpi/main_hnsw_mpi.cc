    ...
    std::vector<int>& assignment) // assignment[i] = rank for vector i
{
    ...
    for(int p = 0; p < nparts; p++){
        ...
        std::memcpy(centroids.data() + p * vecdim, base + idx * vecdim, vecdim * sizeof(float));
    }
    // K-Means 迭代
...
int main(int argc, char* argv[]){
    ...
    std::vector<size_t> my_labels(my_count);
    {
        ...
        for(size_t i = 0; i < base_number; i++){
            if(partition_assignment[i] == rank){
                std::memcpy(my_base.data() + pos * vecdim, base + i * vecdim, vecdim * sizeof(float));
    ...
    if(rank == 0)
        std::cerr << "HNSW built per rank: n=" << my_count << " dim=" << vecdim << " ef=" << ef << "\n";
    // 并行搜索 + recall 计算
