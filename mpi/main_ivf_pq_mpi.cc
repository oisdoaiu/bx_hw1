/*
 * main_ivf_pq_mpi.cc — IVF-PQ MPI 并行化（含 rerank 阶段并行）
 *
 * 环境变量:
 *   IVF_MODE=pq_only | rerank_serial | rerank_omp
 *   IVF_NPROBE (默认 16)
 *   IVF_RERANK (rerank 候选数，默认 200)
 *   NUM_THREADS (rerank_omp 模式下 OpenMP 线程数)
 */
