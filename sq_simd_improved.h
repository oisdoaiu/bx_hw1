...
inline uint32_t InnerProductSQ(const uint8_t* x, const uint8_t* y, size_t vecdim){
    ...
    uint32_t tmp_sse[4];
    _mm_storeu_si128((__m128i*)tmp_sse, sum0);
    uint32_t result = tmp_sse[0] + tmp_sse[1] + tmp_sse[2] + tmp_sse[3];
...
static std::vector<uint8_t> g_base_sq;
static std::vector<uint32_t> g_sum_qi;
static float g_gmin, g_gscale;
...
{
    size_t p = 50;
    const char* env = std::getenv("SQ_P");
    if(env) p = (size_t)std::atoi(env);
    return sq_search(base, g_base_sq.data(), g_sum_qi.data(),
                     query, g_gmin, g_gscale, base_number, vecdim, k, p);
}
